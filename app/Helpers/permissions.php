...
function check_user_permissions($request, $actionName = NULL, $id = NULL) {
    ...
    foreach ($crudPermissionsMap as $permission => $methods) {
        // if the current method exists in methods list, we'll check the permission
        if (in_array($method, $methods) && isset($classesMap[$controller])) {
            ...
            if ($className == 'post' && in_array($method, ['edit', 'update', 'destroy', 'restore', 'forceDestroy'])) {
                ...
                // if the current user has not update-others-post/delete-others-post permission make sure she/he only modify his/her own post
